...
"@ | Out-File -FilePath "alembic\script.py.mako" -Encoding UTF8

Write-Host "✅ Alembic configuration created" -ForegroundColor Green
...
def downgrade() -> None:
    ...
    op.drop_table('users')
"@ | Out-File -FilePath "alembic\versions\001_initial_migration.py" -Encoding UTF8
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "  - messages"
    Write-Host "  - finetune_jobs"
    Write-Host "  - finetune_datasets"
    Write-Host "  - code_blocks"
    Write-Host "  - system_logs"
    Write-Host "  - cache_entries"
} else {
